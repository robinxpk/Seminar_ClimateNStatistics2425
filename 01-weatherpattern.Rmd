...
In this study, we successfully implemented a CNN-based deep learning approach for classifying large-scale atmospheric CTs over Europe, building upon the established HB framework. Using high-resolution ERA5 data for model training, our method achieved an overall accuracy of 53.17% and a macro F1-score of 55.14%, clearly outperforming traditional classification approaches. The CNN model effectively learned dominant circulation patterns, yet faced challenges classifying rare or inherently complex CTs. These limitations indicate potential issues of class imbalance and inherent labeling uncertainties associated with manual classifications.
...
The results, are illustrated in the presentation, demonstrated noticeably different classification performance compared to the setup here. However, it's relatively closer to the findings of the paper. In particular, I suspect that the size of the fully connected linear layers leads to this difference. Since in earlier experiments with narrower range of hyperparameters, we tuned the fully connected layer size to fixed 50 dimensions, however, in the latest optimization round, where more extensive hyperparameter tuning were applied, the model consistently favored a dimension size of 128, which leads to high accuracy for each HB CTs. Therefore, the model shows good performance on the current dataset, it is important to note that this does not necessarily guarantee the absence of overfitting. 
...
The classification used in this study is based on a subjective approach, and it is crucial to quantify the potential human-level errors in labeling. This could provide valuable insight into the accuracy and reliability of the classification system used.

One potential improvement to the statistical analysis would be to reduce the number of subdivisions of circulation types. For example, subdividing the Großwetterlagen according to the wind direction of the jet stream could increase the robustness of the results. Additionally, the study found that the most frequent misclassifications occurred between pairs of anticyclonic and cyclonic circulation types, which could be addressed through refined classification methods.
...
Boxplot of absolute future change and  F1 score of paper (Figure \@ref(fig:boxpaperabs))
