...
Applied to the CanESM2 ensemble projections, which simulate observed past climate, our results show significant future frequency shifts in circulation types, notably an increase in Icelandic High, Cyclonic (HNZ) occurrences during both summer and winter half-years. This methodological advancement not only enhances classification accuracy but also offers robust ensemble analyses critical for climate-informed decision-making.
...
@Bissolli2001 describes a relationship between frequency and duration of the HB circulation types and mean annual air temperature and precipitation. 
Moreover, extreme weather events—such as heavy rainfall, floods, and heat waves—can often be linked to specific HB circulation types [@Mittermeier2022]. 
...
The difference of ERA5 (Figure \@ref(fig:ERA5)) and ERA-20C (Figure \@ref(fig:ERA20C)) lies on: 
...
2. SMHI-LENS [@Wyser2021]
    ...
    * Scenario: SSP3-7.0, representing a medium-to-high greenhouse gas emission pathway
    * Temporal Coverage:
        * Historical Reference: 1991–2020
        * Future Projections: 2071–2100
    * Spatial Resolution: Around 0.7°
     
As a result with higher greenhouse gas emission under SSP3-7.0 scenario, and with a more distant future in the paper:

1. Future warming and circulation shifts might be more pronounced than our implementation.
2. Frequency changes in circulation types may appear larger in the paper, simply because the climate change signal is stronger.
...
4. Fully Connected Layers: Two fully connected layers following the convolutional and dropout layers, used for integrating learned spatial features into final predictions of circulation types.
...
2. Produce physically consistent and stable time series of circulation types.
3. Ensure that predictions align with domain-specific expectations.

According to the original paper:

1. Step 1: Identify all transitions where the predicted circulation type lasts fewer than three days.
2. Step 2: Check neighborhood consistency:
    * If the circulation type before and after the short transition is the same, assign this type to the transition days.
...
macro F1-score, and overall accuracy, following the approach used in the reference study [@Mittermeier2022].
...
The confusion matrix (Appendix Figure \@ref(fig:conf)) shows that most misclassifications occur among CTs with similar dynamical structures, consistent with expectations from atmospheric science. Diagonal dominance is visible but moderate, reflecting the challenging nature of this classification task.


Compared to the paper results, which reported an overall accuracy of 41.1% and a macro F1-score of 38.3%,
...
To further evaluate the quality of the predictions, the RMSE (Root-Mean-Square Error) between the predicted circulation type composites and the true label composites was calculated for variable SLP. 
Additionally, RMSEs for false positives and false negatives were computed separately.

The average RMSE for our implementation’s predictions across all circulation types is 0.88,
...
Our implementation achieves a lower prediction RMSE (0.74) compared to the paper (0.89).
This indicates that, on average, the spatial patterns of the correctly predicted circulation types are more similar to the true labels in our model than in the reference study.

However, the false positives RMSE is much higher (1.45) than in the paper (1.09).
This suggests that when our implementation wrongly predicts a circulation type, 
the resulting spatial pattern is less similar to the true pattern than in the paper.

Our implementation shows a lower false negatives RMSE (0.69) compared to the paper (1.28).
This means that when the model misses a true circulation type, 
...
Figure \@ref(fig:sig) shows the signature plots for four circulation types (CTs) selected based on our RMSE results:

1. Two CTs with lowest RMSE: WZ and TRW (best performance)
2. Two CTs with highest RMSE: NA and HNFA (worst performance)

For each CT, four different composite plots are displayed:

1. Labels: Composite of true labels.
2. Predictions: Composite of model predictions.
3. False Positives: Days predicted as the CT but labeled differently.
4. False Negatives: Days labeled as the CT but predicted differently.

RMSE values are shown below each panel, comparing each composite to the true label composite.

```{r sig, fig.cap = 'Signature plot of slp', cache=FALSE, out.width="70%", fig.align="center", echo=FALSE, eval = TRUE}
...
Differences from the original paper (Figure \@ref(fig:sigpaper)) arise primarily from data variations, model differences, and random initialization.

```{r sigpaper,fig.cap = 'Signature plot of slp of the paper', cache=FALSE, out.width="70%", fig.align="center", echo=FALSE, eval = TRUE}
...
1. Some circulation types are easier to predict (low RMSE), others are harder (high RMSE).
2. False positives tend to have higher RMSE than correct predictions.
3. A larger discrepancy in the color distribution indicates a higher complexity of CT, and false negatives vary depending on CT complexity.

Thus, while specific CTs differ, our results fundamentally agree with the conclusions of the paper that certain patterns are systematically easier or harder to classify, and signature plots are effective to visualize these tendencies.

### Frequency Distribution of Circulation Types

Figure \@ref(fig:count) shows the frequency distribution of the 29 circulation types, expressed as the average number of days per year for the period 1950–1980.
For each circulation type, the frequency derived from the true labels (blue bars) is compared to the frequency derived from the network predictions (yellow bars).

```{r count,fig.cap = 'CTs frequency (implementation)', cache=FALSE, out.width="70%", fig.align="center", echo=FALSE, eval = TRUE}
knitr::include_graphics('work/01-weatherpattern/figures/daycount.png')
```

Overall, the CNN reliably reproduces observed CT frequencies:

1. Dominant CTs (e.g., WZ, HM) are captured accurately, reflecting robust model learning.
2. Rarer CTs (e.g., NA, SZ) exhibit minor deviations, suggesting challenges in accurately learning patterns due to fewer examples, which may indicate slight overfitting.

```{r countpaper,fig.cap = 'CTs frequency (paper)', cache=FALSE, out.width="70%", fig.align="center", echo=FALSE, eval = TRUE}
knitr::include_graphics('work/01-weatherpattern/figures/paper/daycount.png')
```

In comparison with the original study (Figure \@ref(fig:countpaper)), despite differences in dataset periods (our study: 1950–1980; original: 1900–1980), frequency patterns remain consistent. Slight discrepancies in rarer CTs are likely due to dataset differences, emphasizing the importance of extended or more balanced training datasets for better generalization.

The agreement between the label frequencies and the network predictions, as well as the similarity to the paper’s findings, indicates that our model effectively learns the relative occurrence rates of the different circulation types.

Overall, the frequency distribution results support the conclusions of the original study, confirming that the network can reliably reproduce the occurrence rates of various circulation types.

### Future Change

Figure \@ref(fig:boxn) and Figure \@ref(fig:box) show the relative changes in the frequency of occurrence of the 29 circulation types between the future period (2031–2060) and the historical reference period (1971–2000), for the entire year, winter half-year, and summer half-year, based on 12 climate model realizations.
...
The grouping by wind direction allows visual comparison of performance across different CT categories.

```{r boxn,fig.cap = 'Boxplot of freq change and f1 score without transitional smoothing', cache=FALSE, out.width="70%", fig.align="center", echo=FALSE, eval = TRUE}
knitr::include_graphics('work/01-weatherpattern/figures/non_s_combined_boxplot.png')
```
  

```{r box,fig.cap = 'Boxplot of freq change and f1 score after transitional smoothing', cache=FALSE, out.width="70%", fig.align="center", echo=FALSE, eval = TRUE}
...
1. The boxplots show relatively smaller spreads for most circulation types.
...
1. The boxplots show larger spreads, particularly for rare circulation types.
...
Our findings reaffirm the value of multi-model ensembles and careful consideration of methodological choices when projecting future circulation trends, 
...
```{r boxpaper,fig.cap = 'Boxplot of freq change and f1 score of paper', cache=FALSE, out.width="70%", fig.align="center", echo=FALSE, eval = TRUE}
knitr::include_graphics('work/01-weatherpattern/figures/paper/boxplot.png')
```

The original study (Figure \@ref(fig:boxpaper)) shows relatively controlled variability, mostly within ±50%, and fewer extreme outliers, indicating relatively stable and moderate shifts in circulation types.
Our results (Figure \@ref(fig:boxn)) exhibit larger variability, particularly notable in specific circulation types such as NA, SEA, HNFZ, and SZ, with frequency changes frequently exceeding ±50%, even reaching ±100%.

1. Entire Year:
    *	Original study shows moderate shifts centered around zero, with clear signals for types like WA and SEA.
...
2. Winter Half-Year:
	  *	Original study depicts clear moderate increases for WW and HFA with relatively smaller variability.
...
3. Summer Half-Year:
	  *	Original data indicate mild variability and clear, moderate increases and decreases in specific types (e.g., decreases in HNFZ, increases in WA).
...
Key Circulation Type Differences:

1. WA (Westerly Anticyclonic):
	  *	Original study consistently shows WA increasing clearly across all seasons.
...
2. HNZ (Icelandic High Cyclonic):
	  ...
	  *	Original study presents less dramatic decreasing changes for HNZ, reflecting a critical methodological or scenario-driven difference.
3. NA and SZ (Northerly Anticyclonic and Southerly Cyclonic):
	  ...
	  *	Original study maintains relatively modest decreasing changes for these types.
...
2. Our model demonstrates much higher stability with F1-scores generally between 0.80–0.95, suggesting improved reliability and robustness in identifying circulation patterns, despite higher variability in frequency projections.
...
These differences underline the sensitivity of circulation type projections to methodological choices, highlighting the importance of transparent documentation of scenario assumptions and model details to ensure the accurate interpretation and comparability of future climate studies.
...
Our analysis of future projections based on the CanESM2 model (historical) highlights significant shifts in circulation type frequencies, particularly an increased frequency of certain CTs such as Icelandic High Cyclonic (HNZ) during both summer and winter half-years. Notably, our results differ from previous studies, primarily due to distinct methodological choices such as dataset selection, scenario assumptions, and smoothing techniques. These variations underline the sensitivity of circulation type projections and stress the importance of transparent methodological reporting.
...
CTs (Figure \@ref(fig:cts))

```{r cts,fig.cap = 'Circulation types', cache=FALSE, out.width="70%", fig.align="center", echo=FALSE, eval = TRUE}
