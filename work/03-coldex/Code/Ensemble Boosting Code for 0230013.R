...
nc_dir <- "C:/Users/Kathrin/Documents/Dokumente/Master/WiSe24_25/Statistik/BSSP370cmip6.0230013/BSSP370cmip6.0230013"
...
coldest_min_5 <- min_results %>%
  arrange(min_temp) %>%
  head(3)

# Extract files of the coldest minima and average values
coldest_avg_files <- coldest_avg_5$file
coldest_min_files <- coldest_min_5$file

# Mark in the entire dataset
all_results <- all_results %>%
  mutate(is_coldest_avg = ifelse(file %in% coldest_avg_files, "Coldest Avg 5", "All others"),
         is_coldest_min = ifelse(file %in% coldest_min_files, "Coldest Min 5", "All others"))
...
ggplot(all_results, aes(x = date, y = temperature, group = file, color = is_coldest_avg)) +
  ...
  geom_line(data = all_results %>% filter(is_coldest_avg == "Coldest Avg 5"), aes(color = ensemble), size = 1) +
  scale_color_manual(values = c("orange", "red", "blue", "green", "purple")) +
  labs(title = "Temperature Time Series (5 Coldest Average Highlighted)",
       y = "Temperature (°C)",
       color = "Ensemble") +
  theme_classic()
...
ggplot(all_results, aes(x = date, y = temperature, group = file, color = is_coldest_min)) +
  ...
  geom_line(data = all_results %>% filter(is_coldest_min == "Coldest Min 5"), aes(color = ensemble), size = 1) +
  scale_color_manual(values = c("orange", "red", "blue", "green", "purple")) +
  labs(title = "Temperature Time Series (5 Coldest Minima Highlighted)",
       y = "Temperature (°C)",
       color = "Ensemble") +
  theme_classic()
...
cat("\nSummary statistics for the 5 coldest average storylines:\n")
coldest_avg_5 %>%
  print()

cat("\nSummary statistics for the 5 coldest minima storylines:\n")
coldest_min_5 %>%
  print()
