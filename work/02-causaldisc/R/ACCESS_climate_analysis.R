#### ACCESS CLIMATE PATTERN ANALYSIS ######
...
writeCDF(access_regridded, "./data/ACCESS/ACCESS-ESM1-5_1950-1990_regridded.nc", 
         overwrite = TRUE)

#### CALCULATE SPI (STANDARDIZED PRECIPITATION INDEX) ####
raster_data <- access_regridded
...
writeCDF(spi_raster, "./data/ACCESS/SPI12_ACCESS-ESM1-5_1950-1990.nc", 
         overwrite = TRUE, varname = "spi12", 
         longname = "SPI-12", zname = "time")

#### CROP POLAR REGIONS ####
spi_access_full <- rast("./data/ACCESS/SPI12_ACCESS-ESM1-5_1950-1990.nc")
print(ext(spi_access_full))

spi_access_rotated <- rotate(spi_access_full)
print(ext(spi_access_rotated))

spi_no_poles <- crop(spi_access_rotated, ext(-180, 180, -70, 70))
print(ext(spi_no_poles))

writeCDF(spi_no_poles, "./data/ACCESS/SPI12_ACCESS_no_poles.nc", 
         overwrite = TRUE, varname = "spi12", 
         longname = "SPI-12", zname = "time")

spi_access_cropped <- rast("./data/ACCESS/SPI12_ACCESS_no_poles.nc")
print(ext(spi_access_cropped))

jpeg(filename = "./output/spi_access_plot.jpg", 
     width = 1200, height = 800, quality = 100, res = 120)
plot(spi_access_cropped[[20]], main = "SPI-12 ACCESS (Time Step 20) - Poles Removed")
dev.off()


#### PCA AND VARIMAX ROTATION ANALYSIS ####
spi_data_cropped <- as.array(spi_access_cropped)
...
coords <- xyFromCell(spi_access_cropped, row_indices)
...
write.csv(subset_data, "./output/ACCESS_subset_rotated_scores.csv", row.names = FALSE)
...
key_components <- c(1, 2, 3, 7, 13, 21)
...
ggsave("./output/ACCESS_varimax_rotation_variance_adjusted_scale.jpeg", 
...
windows()
jpeg(filename = "./output/ACCESS_key_climate_components.jpg", 
     width = 1200, height = 800, quality = 100, res = 120)
par(mar = c(4, 4, 4, 8))
...
jpeg(filename = "./output/ACCESS_component_maps.jpg", 
...
selected_components <- c(1, 2, 3, 7, 13, 21)
...
write.csv(selected_data, "./output/ACCESS_selected_components.csv", row.names = FALSE)