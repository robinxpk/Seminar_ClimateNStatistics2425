...
Mention that the probabiliy of course depends on the exact variable values and 
that probability can be low due to one variable being extreme oder so
Thus, the next section uses HQs to check most probable stuff 

After mainly describing our fitted models, we now 
examine how flood event probabilities differ between a univariate and multivariate approache. 
It is common in hydrology to fit a GEV distribution to flood peak values and 
...
The following focuses on the flood in Munich 2024 to showcase these differences.

Figure \@ref(probUni) displays the marginal fit of a GEV distribution to peak data of the Munich station. 
...
```
The x-axis of this plot denotes peak values, the y-axis the density of the fitted GEV model. 
The histogram displays the original data to which the GEV distribution is fit. The resulting smooth distribution 
is marked by the blue line. The red vertical line and the shaded area are the visual display of the probability
for the flood event in Munich 2024.
During this flood, the discharge reached a peak of $462$$[m^3/s]$ which corresponds to a probability of 
$19\%$ according to the fitted GEV distribution. Thereby, this approach assigns the a 
return period of such a peak to be $\frac{1}{0.19} \approx5$ years.

Additional to the flood peak, we also feed volume and duration values into our trivariate copula model.
The following figure \@ref(fig:probMulti) is based on figure \@ref(visualGoF) and visualizes these observed values 
in pseudo-observation scale.
...
```
The red lines in the figure mark the observed pseudo observations for the referenced flood event.


Thereby, this results shows that characterizing a flood event by only its peak may be misleading. Because
a merely univariate approach neglects additional features that are of importance.

Univariate only considers peak, but this does not sufficiently characterizes a flood
While the marginal probability of sucha peak is pretty large, the more complex condieration 
of the flood shows that it was one of the more seldome floods 
ODER SO
...
Discuss low probability and implies flood with this combination of variables only appears 
every 100years according to our vine model
Based on figure \@ref(fig:probMulti), the especially the observed volume value is small. 
